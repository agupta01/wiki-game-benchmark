...
from src.eval import ArticleNotFound, get_next_article
...
class TestGetNextArticle:
    ...
    def test_get_next_article_success(self):
        ...
        # Get next article
        next_page = get_next_article(current_page, mock_invoke, self.wiki_data)
        ...
        # Verify invoke was called with current page
        mock_invoke.assert_called_once_with(current_page)
    ...
    def test_get_next_article_not_found_in_index(self):
        ...
        with pytest.raises(
            ArticleNotFound, match="Article 'Nonexistent Article' not found in index"
        ):
            get_next_article(current_page, mock_invoke, self.wiki_data)
    ...
    def test_get_next_article_file_not_found(self):
        ...
        # Should raise ArticleNotFound
        with pytest.raises(ArticleNotFound, match="File '/nonexistent/path/wiki_99' not found"):
            get_next_article(current_page, mock_invoke, self.wiki_data)
    ...
    def test_get_next_article_empty_file(self):
        ...
        # Should raise ArticleNotFound
        with pytest.raises(ArticleNotFound, match="No article found in file"):
            get_next_article(current_page, mock_invoke, self.wiki_data)
    ...
    def test_get_next_article_malformed_json(self):
        ...
        # Should raise ArticleNotFound
        with pytest.raises(ArticleNotFound, match="Invalid JSON in file"):
            get_next_article(current_page, mock_invoke, self.wiki_data)
    ...
    def test_get_next_article_missing_title_field(self):
        ...
        with open(incomplete_file, "w") as f:
            f.write(json.dumps({"text": "Some content without title"}) + "\n")
        ...
        # Should raise ArticleNotFound
        with pytest.raises(ArticleNotFound, match="Invalid article data in file"):
            get_next_article(current_page, mock_invoke, self.wiki_data)
    ...
    def test_get_next_article_multiline_jsonl(self):
        ...
        with open(multi_file, "w") as f:
            f.write(json.dumps({"title": "First Article", "text": "First content"}) + "\n")
            f.write(json.dumps({"title": "Second Article", "text": "Second content"}) + "\n")

        # Add mapping to multi-line file
        env = lmdb.open(self.db_path, readonly=False)
        with env.begin(write=True) as txn:
            txn.put(b"Multi Article", multi_file.encode())
        env.close()

        mock_invoke = Mock(return_value="Multi Article")
